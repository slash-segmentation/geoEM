...
// because it depends on Polyhedron::make_hole which deletes stuff which is not supported with
// vectors. So we provide a minimal version here which works with a mix of triangles and quads
// (which are by far the most common).

void triangulate_polyhedron(Polyhedron3* P)
{
	// Assumptions:
	// is_valid(), is_not_degenerate(P) [and all of its assumptions]
	Polyhedron3::Facet_iterator end = P->facets_end();
	for (Polyhedron3::Facet_iterator f = P->facets_begin(); f != end; ++f)
	{
		if (f->is_quad())
		{
			// Quad is the most common non-triangle case and we can calculate it very easily
			const Polyhedron3::Halfedge_handle &a = f->halfedge(), &b = a->next(), &c = b->next(), &d = c->next();
			const Point3 &A3 = a->vertex()->point(), &B3 = b->vertex()->point(), &C3 = c->vertex()->point(), &D3 = d->vertex()->point();
			const Plane3 h = Plane3(A3, B3, C3);
			const Point2 A = h.to_2d(A3), B = h.to_2d(B3), C = h.to_2d(C3), D = h.to_2d(D3);
			const Line2 AC = Line2(A, C), BD = Line2(B, D);
			const CGAL::Sign A_side = BD.oriented_side(A), B_side = AC.oriented_side(B), C_side = BD.oriented_side(C), D_side = AC.oriented_side(D);
			if      (A_side == CGAL::ON_ORIENTED_BOUNDARY || C_side == CGAL::ON_ORIENTED_BOUNDARY) { P->split_facet(a, c); }
			else if (B_side == CGAL::ON_ORIENTED_BOUNDARY || D_side == CGAL::ON_ORIENTED_BOUNDARY) { P->split_facet(b, d); }
			else if (B_side != D_side) { P->split_facet(a, c); }
			else                       { P->split_facet(b, d); }
		}
		else if (!f->is_triangle())
		{
			//Polygon2& p = facet_to_polygon2(f);
			//if (p.is_clockwise_oriented()) { p.reverse_orientation(); } // force counter-clockwise
			//std::list<Polygon2> partition_polys;
			//CGAL::approx_convex_partition_2(p.vertices_begin(), p.vertices_end(), std::back_inserter(partition_polys));
			throw std::invalid_argument("Cannot triangulate polyhedrons with any facets of degree >4 unless compiled with Polyhedrons using lists");
		}
	}
}
